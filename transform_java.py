...
if __name__ == "__main__":
    ...
    for split in splits:
        with open('./dataset/{}.jsonl'.format(split)) as f:
            for idx, line in enumerate(f.readlines()):
                as_json = json.loads(line)
                the_code = as_json["function"]
    ...
    for changed, split, t_name, the_hash, code, site_map, as_json in itertools.chain.from_iterable(tqdm.tqdm(results, desc="    + Progress", total=len(tasks))):
        ...
        as_json["adv_func"] = code
        as_json["idx"] = the_hash
        as_json["site_map"] = site_map
        # 3c46dca803d24e715d85de678c59dcfac9525a00
        # if the_hash == "3c46dca803d24e715d85de678c59dcfac9525a00":
        #     print()
    ...
    for t_name in all_sites:
        for split in all_sites[t_name]:
            if not os.path.exists('./dataset/'+t_name):
                os.makedirs('./dataset/'+t_name)
            with open('./dataset/{}/adv_{}_site_map.json'.format(t_name, split), 'w') as f:
    ...
    for t_name in all_results:
        for split in all_results[t_name]:
            if not os.path.exists('./dataset/'+t_name):
                os.makedirs('./dataset/'+t_name)
            with open('./dataset/{}/adv_{}.jsonl'.format(t_name, split), 'w') as f:
