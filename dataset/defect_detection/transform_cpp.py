...
def t_rename_func(the_code, uid=1, all_sites=False):
    ...
    func = [f.source for f in func]
    func = the_ast.get_selected_identitier(func)
...
def t_insert_print_statements(the_code, uid=1, all_sites=False):
    ...
    for name in statements_dict:
        for p in statements_dict[name]:
            ...
            if first:
                first = False
                site_map["printf(\"@R_{}@\");".format(uid+count)] = ("", "transforms.InsertPrintStatements")
                code_list[p] = "printf(\"REPLACEME{}\");\n".format(uid+count) + "    "*deep + code_list[p]
                count += 1
            site_map["printf(\"@R_{}@\");".format(uid+count)] = ("", "transforms.InsertPrintStatements")
            code_list[p] = code_list[p] + "\n"+"    "*deep+"printf(\"REPLACEME{}\");\n".format(uid+count)+"    "*deep
        count += 1
...
def process(item):
    ...
    # transforms.append(('transforms.RenameFields', t_seq([t_rename_fields], all_sites=True)))
    # transforms.append(('transforms.RenameParameters', t_seq([t_rename_parameters], all_sites=True)))
    # transforms.append(('transforms.RenameLocalVariables', t_seq([t_rename_local_variables], all_sites=True)))
    # transforms.append(('transforms.AddDeadCode', t_seq([t_add_dead_code], all_sites=True)))
    # transforms.append(('transforms.InsertPrintStatements', t_seq([t_insert_print_statements], all_sites=True)))
    # transforms.append(('transforms.ReplaceTrueFalse', t_seq([t_replace_true_false], all_sites=True)))
    # transforms.append(('transforms.ReplaceFuncName', t_seq([t_rename_func], all_sites=True)))
...
if __name__ == "__main__":
    ...
    for split in splits:
        with open('./{}.jsonl'.format(split)) as f:
            for line in f.readlines():
                ...
                the_code = as_json['func']
                the_code = AST.remove_comments_and_docstrings(the_code, "cpp")
                tasks.append((split, as_json['idx'], the_code, as_json))
                # if len(tasks) > 10: break
            
    print("    + Loaded {} transform tasks".format(len(tasks)))
    results = pool.imap_unordered(process, tasks, 3000)
    results = [process(task) for task in tqdm.tqdm(tasks)]
    ...
    for changed, split, t_name, the_hash, code, site_map, as_json in itertools.chain.from_iterable(tqdm.tqdm(results, desc="    + Progress", total=len(tasks))):
        ...
        adv_json = {}
        adv_json["code"] = as_json["func"]
        adv_json["label"] = as_json["target"]
        adv_json["idx"] = as_json["idx"]
