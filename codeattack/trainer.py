...
import codeattack
from codeattack.shared.utils import logger
...
class Trainer:
    ...
    def _generate_adversarial_examples(self, epoch):
        ...
        adversarial_dataset = codeattack.datasets.Dataset(
            adversarial_examples,
    ...
    def training_step(self, model, tokenizer, batch):
        ...
        _targets = targets
        targets = targets.to(codeattack.shared.utils.device)
        ...
        if isinstance(model, transformers.PreTrainedModel) or (
            ...
            input_ids.to(codeattack.shared.utils.device)
            logits = model(**input_ids)[0]
        else:
            input_ids = tokenizer(input_texts)
            if not isinstance(input_ids, torch.Tensor):
                input_ids = torch.tensor(input_ids)
            input_ids = input_ids.to(codeattack.shared.utils.device)
        ...
        sample_weights = torch.ones(
            is_adv_sample.size(), device=codeattack.shared.utils.device
        )
    ...
    def train(self):
        ...
        codeattack.shared.utils.set_seed(self.training_args.random_seed)
        ...
        model.to(codeattack.shared.utils.device)
    ...
    def _write_readme(self, best_eval_score, best_eval_score_epoch, train_batch_size):
        ...
        if isinstance(
            self.train_dataset, codeattack.datasets.HuggingFaceDataset
            ...
            self.eval_dataset, codeattack.datasets.HuggingFaceDataset
