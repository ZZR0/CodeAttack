...
class BLEU(Metric):
    ...
    def calculate(self, results):
        ...
        self.all_metrics["original_bleu4"] = self.cal(ori_results)
        self.all_metrics["perturbed_bleu4"] = self.cal(per_results)
    ...
    def cal(self, results):
        ...
        for result in results:
            golds.append(result.ground_truth_output)
            predictions.append(result.raw_output)
        (goldMap, predictionMap) = computeMaps(predictions, golds) 
        bleu4 = round(bleuFromMaps(goldMap, predictionMap)[0],2)
        return bleu4
