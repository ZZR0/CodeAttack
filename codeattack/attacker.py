...
class Attacker:
    ...
    def _attack(self):
        ...
        while worklist:
            ...
            if (
                ...
                new_checkpoint = codeattack.shared.AttackCheckpoint(
                    self.attack_args,
    ...
    def attack_dataset(self):
        ...
        codeattack.shared.utils.set_seed(self.attack_args.random_seed)
    ...
    def from_checkpoint(cls, attack, dataset, checkpoint):
        ...
        """
        assert isinstance(
            checkpoint, (str, codeattack.shared.AttackCheckpoint)
        ), f"`checkpoint` must be of type `str` or `textattack.shared.AttackCheckpoint`, but got type `{type(checkpoint)}`."

        if isinstance(checkpoint, str):
            checkpoint = codeattack.shared.AttackCheckpoint.load(checkpoint)
    ...
    def attack_interactive(attack):
        ...
        while True:
            ...
            example = codeattack.shared.attacked_text.AttackedCode(text)
...
def attack_from_queue(
    ...
    codeattack.shared.utils.set_seed(attack_args.random_seed)
