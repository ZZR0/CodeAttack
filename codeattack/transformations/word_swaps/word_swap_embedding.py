...
"""
import random
from codeattack.shared import AbstractWordEmbedding, WordEmbedding
...
class WordSwapEmbedding(WordSwap):
    ...
    def _get_replacement_words(self, word):
        ...
        try:
            ...
            # This word is not in our word embedding database, so return an random list.
            vocab_size = self.embedding.nn_matrix.shape[0]
            nnids = [random.randint(0,vocab_size-1) for _ in range(self.max_candidates)]
            for i, nbr_id in enumerate(nnids):
                nbr_word = self.embedding.index2word(nbr_id)
                candidate_words.append(recover_word_case(nbr_word, word))
            return candidate_words
