...
import codeattack
from codeattack import DatasetArgs, ModelArgs
from codeattack.commands import TextAttackCommand

logger = codeattack.shared.utils.logger


def _cb(s):
    return codeattack.shared.utils.color_text(str(s), color="blue", method="ansi")
...
class EvalModelCommand(TextAttackCommand):
    ...
    def get_preds(self, model, inputs):
        with torch.no_grad():
            preds = codeattack.shared.utils.batch_model_predict(model, inputs)
        return preds
    ...
    def test_model_on_dataset(self, args):
        ...
        while i < min(args.num_examples, len(dataset)):
            ...
            for (text_input, ground_truth_output) in dataset_batch:
                attacked_text = codeattack.shared.AttackedCode(text_input)
    ...
    def run(self, args):
        ...
        codeattack.shared.utils.set_seed(args.random_seed)
