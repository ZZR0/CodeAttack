...
import codeattack
from codeattack.attack_results import (
    FailedAttackResult,
    MaximizedAttackResult,
    SkippedAttackResult,
    SuccessfulAttackResult,
)
from codeattack.constraints import Constraint, PreTransformationConstraint
...
from codeattack.shared import AttackedCode, utils
from codeattack.shared.attacked_code import AttackedCode
from codeattack.transformations import CompositeTransformation, Transformation
...
class Attack:
    ...
    def __init__(
        ...
        for constraint in constraints:
            if isinstance(
                constraint,
                codeattack.constraints.PreTransformationConstraint,
            ):
    ...
    def cuda_(self):
        ...
        def to_cuda(obj):
            visited.add(id(obj))
            if isinstance(obj, torch.nn.Module):
                obj.to(codeattack.shared.utils.device)
            elif isinstance(
                obj,
                (
                    Attack,
    ...
    def _get_transformations_uncached(self, current_text, original_text=None, **kwargs):
        ...
        Args:
            current_text: The current ``AttackedCode`` on which to perform the transformations.
            original_text: The original ``AttackedCode`` from which the attack started.
        Returns:
    ...
    def _filter_transformations_uncached(
        ...
        Args:
            transformed_texts: A list of candidate transformed ``AttackedCode`` to filter.
            current_text: The current ``AttackedCode`` on which the transformation was applied.
            original_text: The original ``AttackedCode`` from which the attack started.
        """
    ...
    def _attack(self, initial_result):
        """Calls the ``SearchMethod`` to perturb the ``AttackedCode`` stored in
        ``initial_result``.
    ...
    def attack(self, example, ground_truth_output):
        ...
        Args:
            example (:obj:`str`, :obj:`OrderedDict[str, str]` or :class:`~textattack.shared.AttackedCode`):
                ...
                Also accepts :class:`~textattack.shared.AttackedCode` that wraps around the input.
        ...
        """
        assert isinstance(
            example, (str, OrderedDict, AttackedCode, AttackedCode)
        ), "`example` must either be `str`, `collections.OrderedDict`, `textattack.shared.AttackedCode`."
        if isinstance(example, (str, OrderedDict)):
            example = AttackedCode(example)
