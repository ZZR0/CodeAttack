...
from codeattack.shared.utils import LazyLoader, device
        ...
        if context_key in self._pos_tag_cache:
            ...
            if self.tagger_type == "flair":
                context_key_sentence = Sentence(
                    context_key, use_tokenizer=codeattack.shared.utils.words_from_text
                )
                self._flair_pos_tagger.predict(context_key_sentence)
                word_list, pos_list = codeattack.shared.utils.zip_flair_result(
                    context_key_sentence
                )

            if self.tagger_type == "stanza":
                word_list, pos_list = codeattack.shared.utils.zip_stanza_result(
