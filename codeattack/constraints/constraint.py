...
import codeattack
from codeattack.shared.utils import default_class_repr
...
class Constraint(ABC):
    ...
    ``AttackedCode`` to another ``AttackedCode`` meet certain conditions.
    ...
    def call_many(self, transformed_texts, reference_text):
        ...
        Args:
            transformed_texts (list[AttackedCode]): The candidate transformed ``AttackedCode``'s.
            reference_text (AttackedCode): The ``AttackedCode`` to compare against.
        """
    ...
    def _check_constraint_many(self, transformed_texts, reference_text):
        ...
        Args:
            transformed_texts (list[AttackedCode]): The candidate transformed ``AttackedCode``
            reference_texts (AttackedCode): The ``AttackedCode`` to compare against.
        """
        return [
            transformed_text
            for transformed_text in transformed_texts
    ...
    def __call__(self, transformed_text, reference_text):
        ...
        Args:
            transformed_text (AttackedCode): The candidate transformed ``AttackedCode``.
            reference_text (AttackedCode): The ``AttackedCode`` to compare against.
        """
        if not isinstance(transformed_text, codeattack.shared.AttackedCode):
            raise TypeError("transformed_text must be of type AttackedCode")
        if not isinstance(reference_text, codeattack.shared.AttackedCode):
            raise TypeError("reference_text must be of type AttackedCode")
    ...
    def _check_constraint(self, transformed_text, reference_text):
        ...
        Args:
            transformed_text: The candidate transformed ``AttackedCode``.
            reference_text (AttackedCode): The ``AttackedCode`` to compare against.
        """
